...
function shot(position, element) {
	...
	setTimeout(() => {
		for (N = 0; N < 1; N++) {
			...
			const positionUserShips = document.getElementById(
				`positionUser-${positionShot}`
			);
			const etiqueta = document.getElementById(
				`positionImgShip-${positionShot}`
			);
			...
			if (
				positionUser.includes(positionShot) == true &&
				positionUsed.includes(positionShot) == false
			) {
				...
				if (shipsUser == 0) {
					...
					txtResult.innerHTML =
						'La computadora destruyó todos tus barcos, Perdiste !';
					btnPlayAgain.style.display = 'block';
				...
				positionUser.includes(positionShot) == false &&
				positionUsed.includes(positionShot) == false
			) {
				//Modifica la nueva imagen y el tamaño
